...
export function renderTarjeta(pagina) {
  ...
  if (tipo === "proximo") {
    return `<div class="card-inner empty-album-card"><p class="memory-label">NUESTRO ÁLBUM</p><button class="empty-album-symbol final-melody-heart" type="button" data-final-heart aria-label="Corazón">♡</button><h2>${escapar(pagina.titulo)}</h2><p>${escapar(pagina.descripcion)}</p></div>`;
  }
