...
import {
  ...
  guardarPortadaFirebase, reemplazarGaleriaPortadasFirebase, guardarAjustesPortadaFirebase, obtenerPortadaFirebase, restaurarPortadaFirebase,
...
async function selectCover(event) { const files = [...(event.target.files || [])]; if (!files.length) return; try { setCoverStatus(`Preparando ${files.length} ${files.length === 1 ? "foto" : "fotos"}…`); pendingCoverImages = []; for (const file of files) pendingCoverImages.push(await compressImage(file)); pendingCoverImage = pendingCoverImages[0]; document.getElementById("coverPreviewImage").src = pendingCoverImage; document.getElementById("coverGalleryCount").textContent = `${pendingCoverImages.length} ${pendingCoverImages.length === 1 ? "imagen seleccionada" : "imágenes seleccionadas"}.`; updateCoverPreviewStyle(); setCoverStatus("Colección preparada. Pulsa Guardar colección."); } catch (error) { setCoverStatus(friendlyError(error), true); } event.target.value = ""; }
async function publishCover() { const button = document.getElementById("saveCover"); button.disabled = true; setCoverStatus("Guardando colección…"); try { let urls = []; if (pendingCoverImages.length) urls = await reemplazarGaleriaPortadasFirebase(pendingCoverImages); await guardarAjustesPortadaFirebase(getCoverAdjustments()); if (urls.length) { activeCoverImages = urls; document.getElementById("coverPreviewImage").src = urls[0]; } pendingCoverImage = ""; pendingCoverImages = []; document.getElementById("coverSummary").textContent = urls.length ? `${urls.length} portadas · ${document.getElementById("coverMode").value === "mosaic" ? "mosaico" : "aleatorias"}` : document.getElementById("coverSummary").textContent; document.getElementById("coverGalleryCount").textContent = "La colección está guardada en Firebase."; updateCoverPreviewStyle(); setCoverStatus("Colección actualizada para todos los dispositivos."); } catch (error) { setCoverStatus(friendlyError(error), true); } finally { button.disabled = false; } }
async function restoreCover() { if (!confirm("¿Restaurar la portada original?")) return; try { await restaurarPortadaFirebase(); pendingCoverImage = ""; pendingCoverImages = []; activeCoverImages = []; document.getElementById("coverMode").value = "random"; document.getElementById("coverPreviewImage").src = "../assets/fondo.jpg"; document.getElementById("coverSummary").textContent = "Una portada"; document.getElementById("coverGalleryCount").textContent = "Selecciona varias fotografías para crear una colección."; updateCoverPreviewStyle(); setCoverStatus("Portada original restaurada."); } catch (error) { setCoverStatus(friendlyError(error), true); } }
...
function updateCoverPreviewStyle() { const image = document.getElementById("coverPreviewImage"); const shade = document.getElementById("coverPreviewShade"); const preview = image.parentElement; const values = getCoverAdjustments(); image.style.objectPosition = `center ${values.position}%`; image.style.transform = "none"; image.style.filter = "none"; let mosaic = preview.querySelector(".cover-preview-mosaic"); if (!mosaic) { mosaic = document.createElement("div"); mosaic.className = "cover-preview-mosaic"; preview.insertBefore(mosaic, image); } const sources = pendingCoverImages.length ? pendingCoverImages : activeCoverImages; mosaic.replaceChildren(...(values.mode === "mosaic" ? sources.map(source => { const tile = document.createElement("img"); tile.src = source; tile.alt = ""; return tile; }) : [])); mosaic.hidden = values.mode !== "mosaic" || !sources.length; image.hidden = !mosaic.hidden; shade.style.background = `rgba(0,0,0,${values.shade / 100})`; }
